...
echo 'Dotfiles - "Dwarves Foundation - https://d.foundation/'
...
sudoers_file=/etc/sudoers.d/dwarvesf-dotfiles
# Contents of the sudoers file.
function sudoers_text() {
  cat <<EOF
# This file was created by dwarvesf's dotfiles script on $(date -I)
...
# Ensure that we can actually, like, compile anything.
if [[ $(xcode-select -p &> /dev/null; printf $?) -ne 0 ]] && is_osx; then
...
e_header "Execute copy"
do_stuff copy
e_header "Execute link"
do_stuff link
e_header "Execute config"
do_stuff config
e_header "Execute init"
do_stuff init
